                                             ...
                                             ble_gap_evt_adv_report_t const * const p_adv_report)
{
    ...
    __LOG(LOG_SRC_APP, LOG_LEVEL_DBG2, "Connecting");

    scan_evt.scan_evt_id                    = NRF_BLE_SCAN_EVT_CONNECTING_ERROR;
    scan_evt.params.connecting_err.err_code = err_code;

    __LOG(LOG_SRC_APP, LOG_LEVEL_DBG2, "Connection status: %d", err_code);
                                               ...
                                               uint8_t        const * p_addr)
{
    ...
    __LOG(LOG_SRC_APP, LOG_LEVEL_DBG2, "Filter set on address type %i, address 0x",
                  p_addr_filter[*p_counter].addr_type);

    for (index = 0; index < BLE_GAP_ADDR_LEN; index++)
    {
        __LOG(LOG_SRC_APP, LOG_LEVEL_DBG2, "%x", p_addr_filter[*p_counter].addr[index]);
    }

    __LOG(LOG_SRC_APP, LOG_LEVEL_DBG2, "\n\r");
                                               ...
                                               char           const * p_name)
{
    ...
    __LOG(LOG_SRC_APP, LOG_LEVEL_DBG2, "Adding filter on %s name", p_name);
                                                     ...
                                                     nrf_ble_scan_short_name_t const * p_short_name)
{
    ...
    __LOG(LOG_SRC_APP, LOG_LEVEL_DBG2, "Adding filter on %s name", p_short_name->p_short_name);
                                               ...
                                               ble_uuid_t     const * p_uuid)
{
    ...
    __LOG(LOG_SRC_APP, LOG_LEVEL_DBG2, "Added filter on UUID %x", p_uuid->uuid);
                                                     ...
                                                     uint16_t               appearance)
{
    ...
    __LOG(LOG_SRC_APP, LOG_LEVEL_DBG2, "Added filter on appearance %x", appearance);
    return NRF_SUCCESS;
}
                                       ...
                                       ble_gap_evt_adv_report_t const * const p_adv_report)
{
    ...
    // Resume the scanning.
    UNUSED_RETURN_VALUE(sd_ble_gap_scan_start(NULL));
}
                                    ...
                                    ble_gap_evt_t  const * const p_gap)
{
    ...
    {
        __LOG(LOG_SRC_APP, LOG_LEVEL_DBG2, "BLE_GAP_SCAN_TIMEOUT");
        if (p_scan_ctx->evt_handler != NULL)
        {
...
ret_code_t nrf_ble_scan_start(nrf_ble_scan_t const * const p_scan_ctx)
{
    ...
    {
        __LOG(LOG_SRC_APP, LOG_LEVEL_ERROR, "sd_ble_gap_scan_start returned 0x%x", err_code);
        return (err_code);
    }
    __LOG(LOG_SRC_APP, LOG_LEVEL_DBG2, "Scanning");
                                   ...
                                   ble_gap_scan_params_t const * const p_scan_param)
{
    ...
    __LOG(LOG_SRC_APP, LOG_LEVEL_DBG2, "Scanning parameters have been changed successfully");
